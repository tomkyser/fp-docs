...
│   ├── index.md
│   ├── system.md
│   ├── researcher.md                # Pre-operation code analysis engine (Phase 16)
│   └── planner.md                   # Operation strategy engine (Phase 16)
...
│   │   └── project-config.md         # FP-specific paths (mapping extracted to source-map.json)
...
| researcher | agents/researcher.md | opus | 75 | orange | YES | mod-standards, mod-project |
| planner | agents/planner.md | sonnet | 75 | purple | YES | mod-standards, mod-project, mod-orchestration |

NOTE: The manifest.md says verbosity, index, and system use `model: sonnet`, but the actual agent files all declare `model: opus`. The agent files are the source of truth. The researcher uses `model: opus` for deep code analysis (D-12); the planner uses `model: sonnet` since planning is structured and formulaic (D-12).
...
**Write-capable engines**: orchestrate, modify, citations, api-refs, locals, index, system, researcher, planner
...
|--------|---------|-------------|
| mod-standards | Formatting, naming, structural, depth rules | ALL 11 engines |
| mod-project | FP paths, source-map.json CLI reference, environment | ALL 11 engines |
...
| mod-orchestration | Orchestration rules, delegation thresholds, pipeline phase assignments, team protocol, git serialization rules | orchestrate, planner |

The `modify` engine is the most heavily loaded — it preloads 5 modules directly (mod-standards, mod-project, mod-pipeline, mod-changelog, mod-index) and accesses domain module rules indirectly through the pipeline stages. The `researcher` preloads 2 modules (mod-standards, mod-project). The `planner` preloads 3 modules (mod-standards, mod-project, mod-orchestration) — it needs orchestration thresholds for batching decisions.
...
**mod-project**: FP-specific configuration. Project identity (theme root, docs root, WP-CLI prefix, local URL, PHP namespace), source-to-doc mapping CLI reference (lookups via `source-map.json` and `lib/source-map.cjs`, with 5 representative example rows), appendix cross-reference table, key paths (docs hub, changelog, tracker, index, shapes, flagged concerns), feature enable flags.
...
### Pipeline Phase Delegation (5-Phase Model)

Under the multi-agent orchestration architecture, all operations proceed through a 5-phase delegation model. The existing 8 pipeline stages are unchanged -- Research and Plan are pre-pipeline phases that precede the 8-stage pipeline:

```
Research Phase (researcher) -> Plan Phase (planner) -> Write Phase (specialist, stages 1-3) -> Review Phase (validate, stages 4-5) -> Finalize Phase (orchestrator, stages 6-8)
```

| Phase | Name | Stages | Owner |
|-------|------|--------|-------|
| Research Phase | Pre-operation code analysis | Pre-pipeline | researcher engine (delegated) |
| Plan Phase | Operation strategy design | Pre-pipeline | planner engine (delegated) |
...
In **Delegation Mode** (the default for write operations), the orchestrator first spawns the researcher for source code analysis, then the planner for strategy design. The planner produces a persistent plan file. The orchestrator then assigns the Write Phase to the primary specialist, spawns the validate engine for the Review Phase, and handles the Finalize Phase itself. This ensures git serialization: only the orchestrator commits to the docs repo.

In **Standalone Mode** (when a specialist engine runs outside orchestration), the engine executes all 8 stages itself as before.

For **read-only commands** (audit, verify, sanity-check, test, verbosity-audit), the orchestrator uses a streamlined path: Research and Plan phases still run (for consistent architecture and audit trail), but the specialist runs in standalone mode without Write/Review/Finalize phase splitting.
...
- `researcher.enabled = false` (config): Config-level equivalent of always passing `--no-research`.
- `planner.enabled = false` (config): Config-level skip of Plan Phase. Orchestrator uses legacy 3-phase direct delegation.
...
3. If no watermark and on master (first sync): enumerates all mapped directories from `source-map.json` for initial review
4. Filters to theme-scoped files only
5. Maps changed source files to affected doc files via `source-map.json` (queried through `lib/source-map.cjs`)
...
post-merge / post-rewrite hook (codebase .git/hooks/)
    ...
    │  Maps changed source files to affected docs via source-map.json (lib/source-map.cjs)
    │
    ▼
...
The drift analyzer uses `source-map.json` (accessed via `lib/source-map.cjs`) to map changed source files to affected documentation targets. This is the same mapping used by engines for auto-update detection -- `source-map.json` is the single source of truth for all source-to-doc mapping.
...
**Section 9 — Agent Model Configuration** (Phase 16):
- `researcher.model`: opus (deep code analysis benefits from strongest reasoning, D-12)
- `researcher.enabled`: true (master switch for pre-operation Research Phase)
- `planner.model`: sonnet (planning is structured/formulaic, D-12)
- `planner.enabled`: true (master switch for pre-operation Plan Phase)
- `plans.auto_prune`: true (auto-prune completed plans older than retention period)
- `plans.retention_days`: 30 (days to retain completed plan files)
- `plans.max_plans`: 200 (maximum plan files to retain)
- Phase Skip Behavior table: `--no-research`, `--plan-only`, `researcher.enabled = false`, `planner.enabled = false`
...
- Source-to-documentation mapping: **Extracted to `source-map.json`** (accessed via `lib/source-map.cjs`). project-config.md retains a reference pointer only.
...
- **Engine table**: all 11 engines with agent file, model, operations
...
3. **Spawns the researcher** for pre-operation source code analysis (unless `--no-research` or disabled via config)
4. **Spawns the planner** to design execution strategy and produce a persistent plan file
5. **Executes the plan** by delegating to specialist engine(s) with specific pipeline phase assignments
6. **Aggregates results** and returns a unified report to the user

### 5-Phase Delegation Model

All operations proceed through a 5-phase model. The existing 8 pipeline stages are unchanged -- Research and Plan are pre-pipeline phases:

```
Research Phase (researcher) -> Plan Phase (planner) -> Write Phase (specialist, stages 1-3) -> Review Phase (validate, stages 4-5) -> Finalize Phase (orchestrator, stages 6-8)
```

**Research Phase** (assigned to researcher engine):
- Pre-operation source code analysis
- Maps targets to source files, extracts function signatures, hook registrations, dependencies
- Produces structured analysis document at `.fp-docs/analyses/`
- Depth calibrated by operation type: full (write), summary (read-only), minimal (admin)

**Plan Phase** (assigned to planner engine):
- Receives research analysis (or null if `--no-research`)
- Designs execution strategy based on operation type, scope, flags, and analysis
- Creates persistent plan file at `.fp-docs/plans/` via `fp-tools.cjs plans save`
- If `--plan-only`: displays plan summary and stops
...
- Deep code understanding before any documentation work (Research Phase)
- Consistent execution strategy with full audit trail (Plan Phase)
...
- Write operations typically involve 5 agents: orchestrate + researcher + planner + specialist + validate
...
For read-only commands (audit, verify, sanity-check, test, verbosity-audit), the orchestrator uses a streamlined 4-agent path:
1. Orchestrator receives the command
2. Orchestrator spawns researcher for pre-analysis (unless `--no-research` or disabled)
3. Orchestrator spawns planner -- creates minimal 1-phase plan with specialist in standalone mode
4. If `--plan-only`: stops after plan display
5. Orchestrator delegates to the specialist engine (validate or verbosity) in standalone mode
6. Specialist engine executes and returns results
7. Orchestrator returns results to user, updates plan status to completed

No Write/Review/Finalize phase splitting occurs because read-only commands do not modify documentation and do not need changelog/commit stages.
