...
│   ├── fp-docs-api-refs.md
│   ├── fp-docs-locals.md
│   ├── fp-docs-verbosity.md          # Read-only (audit)
│   ├── fp-docs-verbosity-enforcer.md # Write-capable (pipeline stage 1)
│   ├── fp-docs-indexer.md
│   ├── fp-docs-system.md
...
│   ├── auto-revise.md
│   ├── deprecate.md
│   ├── audit.md                      # Read workflow (5-step v2 template)
│   ├── verify.md
│   ├── sanity-check.md
│   ├── test.md
│   ├── verbosity-audit.md
...
├── scaffolds/                        # Bundled scaffold assets for docs repo bootstrap
│   └── user-guide/                   # User guide scaffold (20 files: Hugo Relearn config, layouts, content stubs, templates, LFS, GH Actions)
│       ├── hugo.toml, go.mod         # Hugo Relearn theme configuration
│       ├── layouts/                  # render-image.html, render-link.html, recording.html, step.html
│       ├── content/                  # _index.md root + 6 section stubs + 5 page templates
│       ├── .gitattributes            # Git LFS tracking rules
│       └── .github-workflows/       # -> {docs-root}/.github/workflows/ (deploy action)
...
│   ├── agent-map.cjs                 # Canonical agent name registry + phase authority map
│   ├── scope-assess.cjs              # Pre-delegation scope assessment (complexity, researcher count)
│   ├── tracker.cjs                   # Cross-agent tracker document management (JSON CRUD)
│   ├── merge-intel.cjs               # Docs branch merge intelligence (9 exports: detect, assess, merge, history)
│   ├── migrate.cjs                   # Data layout migration (8 exports: check, migrate, version tracking)
│   └── scaffold.cjs                  # Scaffold bootstrap (4 exports: checkScaffold, bootstrap, listScaffolds, cmdScaffold)
...
├── playwright-mcp-config.json        # Playwright browser config (referenced by .mcp.json)
...
**Write Workflow (10 steps)**: Used by revise, add, auto-update, auto-revise, deprecate, citations (generate/update), api-ref (generate), locals (annotate/contracts/shapes), remediate.
1. Initialize via `fp-tools init write-op`
2. Scope assessment via `fp-tools scope-assess` (determines complexity, researcher count, tracker)
3. Spawn fp-docs-researcher(s) for codebase analysis (1-N based on scope)
4. Spawn fp-docs-planner for execution strategy
5. Spawn primary specialist for Write Phase (primary operation ONLY -- no enforcement)
6. Spawn fp-docs-verbosity-enforcer for Stage 1 (dedicated verbosity enforcement)
7. Spawn fp-docs-citations for Stage 2 (dedicated citation enforcement)
8. Spawn fp-docs-api-refs for Stage 3 (dedicated API reference enforcement)
9. Spawn fp-docs-validator for Review Phase (stages 4-5)
10. Handle Finalize Phase (stages 6-8) via `fp-tools pipeline init/next/run-stage`

**Read Workflow (5 steps)**: Used by audit, verify, sanity-check, test, verbosity-audit.
1. Initialize via `fp-tools init read-op`
2. Scope assessment (determines complexity, researcher count, tracker)
3. Spawn fp-docs-researcher(s) for analysis (1-N based on scope)
4. Spawn fp-docs-planner for strategy
5. Spawn specialist agent for execution (no enforcement pipeline)

**Admin Write Workflow**: Used by update-index, update-claude.
Simplified write workflow: init, primary agent, dedicated enforcement agents (stages 1-3), validator (stages 4-5), finalize (stages 6-8). Skips scope-assess/research.

**Admin No-Pipeline Workflow**: Used by setup, sync, update, update-skills.
Initializes via `fp-tools init admin-op`, spawns the appropriate system agent. No pipeline.
...
| Agent | File | Primary Role | Can Write? |
|-------|------|-------------|------------|
| fp-docs-modifier | fp-docs-modifier.md | Primary write operations (revise, add, auto-update, auto-revise, deprecate) | YES |
| fp-docs-validator | fp-docs-validator.md | Pipeline stages 4-5 (sanity-check, 10-point verification) | NO (disallowed) |
| fp-docs-citations | fp-docs-citations.md | Pipeline stage 2 enforcement + citations generate/update primary | YES |
| fp-docs-api-refs | fp-docs-api-refs.md | Pipeline stage 3 enforcement + api-ref generate primary | YES |
| fp-docs-locals | fp-docs-locals.md | Locals annotate/contracts/shapes primary | YES |
...
Under the workflow-based architecture, all write operations proceed through a 7-phase delegation model with dedicated enforcement agents:

```
Scope-Assess -> Research (1-N) -> Plan -> Write (primary only) -> Enforce (stages 1-3, dedicated agents) -> Review (stages 4-5) -> Finalize (stages 6-8)
```

| Phase | Stages | Owner |
|-------|--------|-------|
| Scope Assessment | Pre-pipeline | `fp-tools scope-assess` (CJS, determines complexity/researcher count/tracker) |
| Research Phase | Pre-pipeline | fp-docs-researcher (1-N spawns based on scope) |
| Plan Phase | Pre-pipeline | fp-docs-planner (spawned by workflow) |
| Write Phase | Primary operation ONLY | Primary specialist (e.g., fp-docs-modifier) -- no enforcement stages |
| Enforce Phase | Stages 1-3 | Dedicated agents: fp-docs-verbosity-enforcer (1), fp-docs-citations (2), fp-docs-api-refs (3) |
| Review Phase | Stages 4-5 | fp-docs-validator (read-only) |
| Finalize Phase | Stages 6-8 | Workflow itself via `fp-tools pipeline init/next/run-stage` |

**Key v2 change**: Stages 1-3 are no longer folded into the primary specialist. Each enforcement stage gets its own dedicated agent spawn. This improves isolation, allows specialized agents to focus on their domain, and enables the primary specialist to stay focused on content.

**Scope assessment** (via `fp-tools scope-assess`) determines: complexity tier (low/medium/high), researcher count (1-N), whether a tracker document is needed, and the delegation plan. Skippable via `--no-research` flag.
...
```bash
fp-tools scope-assess <command> "<arguments>"     # Returns JSON: complexity, researcherCount, targets, trackerRequired, delegationPlan
```

### Tracker Commands

```bash
fp-tools tracker create --command <cmd> --complexity <tier>  # Create tracker document
fp-tools tracker update <id> --step <step> --agent <agent> --status <status> --detail <text>
fp-tools tracker summary <id>                     # Compact summary for agent context
fp-tools tracker complete <id>                    # Close tracker document
fp-tools tracker list                             # List active trackers
fp-tools tracker prune                            # Remove expired trackers
```

### Source Map

```bash
fp-tools source-map lookup <source-path>          # Find doc target for source file
fp-tools source-map reverse <doc-path>            # Find source files for doc
fp-tools source-map stats                         # Map coverage statistics
```

### Scaffold Commands

```bash
fp-tools scaffold list                            # List available scaffolds (name, file count, has workflows)
fp-tools scaffold check <name>                    # Check if scaffold target exists in docs repo
fp-tools scaffold bootstrap <name>                # Bootstrap scaffold into docs repo (skip-if-exists)
```

Bootstrap copies `scaffolds/{name}/` to `{docs-root}/{name}/`, with special handling for `.github-workflows/` which maps to `{docs-root}/.github/workflows/` (repo root, not inside the scaffold subdirectory). Safe for re-runs: only creates files that don't already exist. Also patches the dev wiki's `hugo.toml` `ignoreFiles` to exclude the bootstrapped directory and runs `hugo mod get` + `git lfs install`.
...
fp-tools plans save|load|save-analysis            # Plan and analysis file management
```
...
Configuration at `playwright-mcp-config.json` (plugin root). MCP tools (`browser_navigate`, `browser_snapshot`, etc.) are available to agents as native tool calls.
...
15. **Ephemeral CLI tool pattern**: WP-CLI `fp-locals` command lives in `tools/` but operates in the theme. Setup/teardown managed by `lib/locals-cli.cjs`. SubagentStop safety net auto-cleans orphaned artifacts.
16. **Plugin-bundled MCP**: Browser automation via `.mcp.json` declared Playwright server. No CJS glue code needed.
17. **Auto-bootstrap scaffolding**: Docs repo structures (e.g., `user-guide/`) are bootstrapped automatically from bundled scaffolds in `scaffolds/` via `lib/scaffold.cjs`. Users never manually copy files between repos. Bootstrap is idempotent (skip-if-exists) and handles special placement of GitHub Actions workflows from `.github-workflows/` to `{docs-root}/.github/workflows/`.
