 ...
 *
 * Storage: {docs-root}/.fp-docs/trackers/{id}.json
 ...
 * Prefers docs-root/.fp-docs/trackers, falls back to plugin-root/.fp-docs/trackers.
 *
...
function resolveTrackerDir() {
  try {
    ...
    if (docsInfo.path && docsInfo.exists) {
      return path.join(docsInfo.path, FP_DOCS_DIR, TRACKERS_DIR);
    }
  } catch { /* fall through */ }
...
function create(options) {
  ...
  const tracker = {
    ...
    complexity: (options && options.complexity) || 'light',
    status: 'active',
    created: now,
    updated: now,
    closed: null,
