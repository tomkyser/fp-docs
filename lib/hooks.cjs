...
function cmdHooks(subcommand, args, raw, input) {
  ...
  switch (event) {
    ...
    case 'subagent-stop': {
      if (!matcher) {
        error('Usage: fp-tools hooks run subagent-stop <modify|orchestrate|locals|validate|citations|api-refs|researcher|planner>');
      }
      if (matcher === 'modify') {
        ...
        output(result, raw, result.additionalContext);
      } else if (matcher === 'validate' || matcher === 'citations' || matcher === 'api-refs' || matcher === 'researcher' || matcher === 'planner') {
        const result = handleSubagentEnforcementCheck({ ...input, agent_type: matcher });
        output(result, raw, result.additionalContext);
      } else {
        error(`Unknown subagent-stop matcher: ${matcher}. Use: modify, orchestrate, locals, validate, citations, api-refs, researcher, planner`);
      }
      break;
    }
