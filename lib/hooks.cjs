...
// -- Handler: migration-nudge (Category A) -----------------------------------

/**
 * SessionStart hook: check if old data layout needs migration.
 * Nudges user to run `fp-tools migrate run` if old layout detected.
 * Per spec: "SessionStart nudges but does not auto-migrate."
 *
 * @param {object} input - Parsed stdin JSON (ignored)
 * @returns {{ additionalContext: string }}
 */
function handleMigrationNudge(input) {
  try {
    const codebaseRoot = getCodebaseRoot();
    if (!codebaseRoot) return { additionalContext: '' };

    const docsInfo = getDocsRoot(codebaseRoot);
    if (!docsInfo.path || !docsInfo.exists) return { additionalContext: '' };

    const { needsMigration } = require('./migrate.cjs');
    const check = needsMigration(docsInfo.path, codebaseRoot);
    if (check.needed) {
      return {
        additionalContext: `MIGRATION NEEDED: Old data layout detected (${check.reasons.join(', ')}). Run \`fp-tools migrate run\` to update to the new three-tier layout.`,
      };
    }
  } catch {
...
// -- Handler: scaffold-check (Category A) ------------------------------------

/**
 * SessionStart hook: check for missing scaffolds in docs repo and auto-bootstrap.
 *
 * When the docs repo is reachable and a scaffold target is missing, this handler
 * automatically runs bootstrap() to create the structure. Deterministic and
 * non-interactive — if docs repo is unreachable, silently skips.
 *
 * @param {object} input - Parsed stdin JSON (ignored)
 * @returns {{ additionalContext: string }} Context about bootstrap actions taken
 */
function handleScaffoldCheck(input) {
  try {
    const codebaseRoot = getCodebaseRoot();
    if (!codebaseRoot) return { additionalContext: '' };
    ...
    const { listScaffolds, checkScaffold, bootstrap } = require('./scaffold.cjs');
    const available = listScaffolds();
    if (!available.scaffolds.length) return { additionalContext: '' };

    const actions = [];
    for (const scaffold of available.scaffolds) {
      const status = checkScaffold(scaffold.name, docsInfo.path);
      if (!status.exists) {
        const result = bootstrap(scaffold.name, { docsRoot: docsInfo.path });
        if (result.bootstrapped) {
          actions.push(`Bootstrapped ${scaffold.name}/ (${result.created.length} files)`);
          if (result.postSteps && result.postSteps.length) {
            actions.push(...result.postSteps.map(s => `  - ${s}`));
          }
        } else if (result.error) {
          actions.push(`Scaffold ${scaffold.name}: ${result.error}`);
        }
      }
    }

    if (!actions.length) return { additionalContext: '' };

    return {
      additionalContext: `SCAFFOLD BOOTSTRAP:\n${actions.join('\n')}`,
    };
  } catch {
    return { additionalContext: '' };
  }
