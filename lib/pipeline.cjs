...
const ALGORITHM_PATHS = {
  1: 'framework/algorithms/verbosity-algorithm.md',
  2: 'framework/algorithms/citation-algorithm.md',
  3: 'framework/algorithms/api-ref-algorithm.md',
  4: 'framework/algorithms/validation-algorithm.md',
  5: 'framework/algorithms/validation-algorithm.md',
};
...
function cmdPipeline(subcommand, args, raw) {
  if (!subcommand) {
    error('Usage: fp-tools pipeline <init|next|run-stage|status|reset> [args]');
  }
  ...
  switch (subcommand) {
    ...
    default:
      error('Unknown pipeline subcommand: ' + subcommand + '. Use: init, next, run-stage, status, reset');
  }
}
