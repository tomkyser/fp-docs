 ...
 *
 * Maps changed source files to affected documentation via source-map.cjs
 * abstraction layer, manages staleness signals (append, dedup, clear), and
 * provides nudge formatting for session start notifications.
 *
 * Features:
 * - Drift analysis: git diff -> source-map.cjs mapping -> affected docs
 * - Staleness signals: append-only with dedup by doc_path (highest severity wins) (D-06)
 * - Staleness data in {project-root}/.fp-docs/staleness.json, separate from state.json (D-05)
...
const { getPluginRoot, getCodebaseRoot, getDocsRoot, getGlobalStateRoot } = require('./paths.cjs');
const { loadSourceMap } = require('./source-map.cjs');
...
/**
 * Get the staleness.json path for a given state root.
 *
 * @param {string} stateRoot - The global state root directory
 * @returns {string} Absolute path to staleness.json
 */
function getStalenessPath(stateRoot) {
  return path.join(stateRoot, 'staleness.json');
}

/**
 * Get the drift-pending.json path for a given state root.
 *
 * @param {string} stateRoot - The global state root directory
 * @returns {string} Absolute path to drift-pending.json
 */
function getPendingPath(stateRoot) {
  return path.join(stateRoot, 'drift-pending.json');
}
...
function loadStaleness(stalenessPath) {
  ...
  if (!resolvedPath) {
    ...
    resolvedPath = getStalenessPath(stateRoot);
  }
 ...
 * @param {string} [mapPath] - Optional source-map.json path (for testing)
 * @param {string[]} [mockChangedFiles] - Mock changed files for testing
 * @returns {{ affected_docs: number, signals: Array<object> }}
 */
function analyzeDrift(codebaseRoot, outputPath, mapPath, mockChangedFiles) {
  // Get changed files
  const changedFiles = mockChangedFiles || getChangedFiles(codebaseRoot);

  // Load source-to-doc mapping from source-map.cjs
  const map = loadSourceMap(mapPath);
  const sourceToDocsMap = {};
  for (const m of (map.mappings || [])) {
    if (m.status === 'mapped' && m.doc) {
      sourceToDocsMap[m.source] = m.doc;
    }
  }
...
function mergePending(stalenessPath, pendingPath) {
  ...
  if (!resolvedPendingPath) {
    const stateRoot = resolveStateRoot();
    if (!stateRoot) return { merged: 0 };
    resolvedPendingPath = getPendingPath(stateRoot);
  }
...
function installGitHook(hookName, codebaseRoot, fpToolsPath, fpDocsDir) {
  ...
  const templatePath = path.join(getPluginRoot(), 'templates', hookName + '.sh');
...
function installAllHooks(codebaseRoot) {
  ...
  const fpDocsDir = path.join(codebaseRoot, '.fp-docs');
...
function installShellIntegration(codebaseRoot) {
  ...
  // Read template from plugin templates/
  const templatePath = path.join(getPluginRoot(), 'templates', 'fp-docs-shell.zsh');
  ...
  const content = template
    ...
    .replace(/__DOCS_ROOT__/g, docsInfo.path);
