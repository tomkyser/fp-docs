 ...
 *
 * The routing table has exactly 29 entries matching the full command set.
...
/**
 * The canonical routing table -- exactly 29 entries matching the full command set.
 * Keyed by command name (without /fp-docs: prefix).
 *
 * Each entry has:
 *   agent     - Primary agent name (fp-docs-* prefix, null for meta/orchestrator-handled)
 ...
 */
const ROUTING_TABLE = {
  'revise':          { agent: 'fp-docs-modifier',   workflow: 'revise.md',          operation: 'revise',              type: 'write' },
  'add':             { agent: 'fp-docs-modifier',   workflow: 'add.md',             operation: 'add',                 type: 'write' },
  'auto-update':     { agent: 'fp-docs-modifier',   workflow: 'auto-update.md',     operation: 'auto-update',         type: 'write' },
  'auto-revise':     { agent: 'fp-docs-modifier',   workflow: 'auto-revise.md',     operation: 'auto-revise',         type: 'write' },
  'deprecate':       { agent: 'fp-docs-modifier',   workflow: 'deprecate.md',       operation: 'deprecate',           type: 'write' },
  'audit':           { agent: 'fp-docs-validator',  workflow: 'audit.md',           operation: 'audit',               type: 'read' },
  'verify':          { agent: 'fp-docs-validator',  workflow: 'verify.md',          operation: 'verify',              type: 'read' },
  'sanity-check':    { agent: 'fp-docs-validator',  workflow: 'sanity-check.md',    operation: 'sanity-check',        type: 'read' },
  'test':            { agent: 'fp-docs-validator',  workflow: 'test.md',            operation: 'test',                type: 'read' },
  'citations':       { agent: 'fp-docs-citations',  workflow: 'citations.md',       operation: null,                  type: 'write' },
  'api-ref':         { agent: 'fp-docs-api-refs',   workflow: 'api-ref.md',         operation: null,                  type: 'write' },
  'locals':          { agent: 'fp-docs-locals',     workflow: 'locals.md',          operation: null,                  type: 'write' },
  'verbosity-audit': { agent: 'fp-docs-verbosity',  workflow: 'verbosity-audit.md', operation: 'audit',               type: 'read' },
  'update-index':    { agent: 'fp-docs-indexer',    workflow: 'update-index.md',    operation: 'update-project-index', type: 'admin' },
  'update-claude':   { agent: 'fp-docs-indexer',    workflow: 'update-claude.md',   operation: 'update-example-claude', type: 'admin' },
  'update-skills':   { agent: 'fp-docs-system',     workflow: 'update-skills.md',   operation: 'update-skills',       type: 'admin' },
  'setup':           { agent: 'fp-docs-system',     workflow: 'setup.md',           operation: 'setup',               type: 'admin' },
  'sync':            { agent: 'fp-docs-system',     workflow: 'sync.md',            operation: 'sync',                type: 'admin' },
  'update':          { agent: 'fp-docs-system',     workflow: 'update.md',          operation: 'update',              type: 'admin' },
  'parallel':        { agent: null,                  workflow: 'parallel.md',        operation: 'parallel',            type: 'batch' },
  'remediate':       { agent: null,                  workflow: 'remediate.md',       operation: 'remediate',           type: 'write' },
  'do':              { agent: null,                  workflow: 'do.md',              operation: 'do',                  type: 'meta' },
  'help':            { agent: null,                  workflow: 'help.md',            operation: 'help',                type: 'meta' },
 ...
 *
 * Checks:
 *   1. Command file exists at commands/{command}.md
...
function cmdRoute(subcommand, args, raw) {
  ...
  switch (subcommand) {
    case 'lookup': {
      ...
      output(route, raw, `${route.agent || 'none'}:${route.operation}:${route.type}`);
      break;
    }
...
function cmdHelp(subcommand, raw) {
  if (subcommand === 'grouped') {
    ...
    for (const [type, label] of Object.entries(groupLabels)) {
      ...
      md += `## ${label}\n\n`;
      md += '| Command | Description | Agent |\n';
      md += '|---------|-------------|-------|\n';
      for (const cmd of groups[type]) {
        md += `| ${cmd.command} | ${cmd.description} | ${cmd.agent} |\n`;
      }
      md += '\n';
    }
