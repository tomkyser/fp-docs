 ...
 *
 * Tracks operation log and pipeline state in JSON at {project-root}/.fp-docs/state.json.
 * Features:
...
const { getCodebaseRoot, getDocsRoot, getGlobalStateRoot } = require('./paths.cjs');
...
/**
 * Get the state file path within the global state directory.
 *
 * @param {string} globalRoot - The global state root ({project-root}/.fp-docs/)
 * @returns {string} Absolute path to state.json
 */
function getStatePath(globalRoot) {
  return path.join(globalRoot, 'state.json');
}
...
function loadState(statePath) {
  ...
  // If no explicit path, resolve from global state root
  if (!resolvedPath) {
    ...
    resolvedPath = getStatePath(globalRoot);
  }
...
/**
 * Resolve the default state file path from global state root.
 * Returns null if codebase root is not available.
 *
...
function resolveDefaultStatePath() {
  ...
  return getStatePath(globalRoot);
}
 ...
 * @param {string} [stateRootPath] - Optional state root override (for testing)
 * @returns {string|null} Absolute path to the saved plan file, or null if state root unavailable
 */
function saveRemediationPlan(plan, stateRootPath) {
  const stateRoot = stateRootPath || resolveStateRoot();
  if (!stateRoot) { return null; }
  const plansDir = path.join(stateRoot, 'remediation-plans');
  ensureDir(plansDir);
 ...
 * Otherwise, resolves to {stateRoot}/remediation-plans/{idOrPath}.json.
 *
 ...
 */
function loadRemediationPlan(idOrPath, stateRootPath) {
  let planPath;
  if (path.isAbsolute(idOrPath)) {
    ...
    const stateRoot = stateRootPath || resolveStateRoot();
    if (!stateRoot) return null;
    planPath = path.join(stateRoot, 'remediation-plans', `${idOrPath}.json`);
  }
 ...
 */
function listRemediationPlans(stateRootPath) {
  const stateRoot = stateRootPath || resolveStateRoot();
  if (!stateRoot) return [];
  const plansDir = path.join(stateRoot, 'remediation-plans');
 ...
 */
function updateRemediationPlan(idOrPath, updates, stateRootPath) {
  const plan = loadRemediationPlan(idOrPath, stateRootPath);
  if (!plan) return null;
  ...
  saveRemediationPlan(merged, stateRootPath);
  return merged;
}
