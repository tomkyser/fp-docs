...
function needsMigration(docsRoot, codebaseRoot) {
  ...
  // 1. Old global state at docs root — check for ANY state items, not just state.json
  const oldFpDocs = path.join(docsRoot, '.fp-docs');
  if (fs.existsSync(oldFpDocs)) {
    try {
      const stat = fs.statSync(oldFpDocs);
      if (stat.isDirectory()) {
        const hasOldItems = GLOBAL_STATE_ITEMS.some(item =>
          fs.existsSync(path.join(oldFpDocs, item))
        );
        if (hasOldItems) {
  ...
  // 5. Old docs layout: About.md not yet renamed to README.md
  ...
  // 6. Old docs layout: PROJECT-INDEX.md still in claude-code-docs-system/
...
function cmdMigrate(subcommand, args, raw) {
  if (!subcommand) {
    error('Usage: fp-tools migrate <check|run [--dry-run]|status>');
  }
  ...
  switch (subcommand) {
    ...
    case 'run': {
      ...
      if (dryRun) {
        ...
        const preview = { dryRun: true, wouldMigrate: check.needed, reasons: check.reasons };
        output(preview, raw, JSON.stringify(preview, null, 2));
      } else {
