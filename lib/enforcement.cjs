...
/**
 * Maps agent types to their authorized pipeline phase.
 * Supports both new fp-docs-* agent names (GSD architecture) and legacy
 * engine names (backward compat -- retained for test suite and direct CLI usage).
 * system, indexer, verbosity are administrative/standalone -- no phase authority mapping needed.
 */
const STAGE_AUTHORITY_MAP = {
  // New GSD-style agent names
  ...
  // Legacy engine names (backward compat until Phase 7 hook migration)
  researcher: 'research',
  planner: 'plan',
  modify: 'write',
  citations: 'write',
