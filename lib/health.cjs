...
function runHealthChecks(pluginRoot) {
  ...
  // 5. Routing table: verify all 31 entries exist
  ...
  if (entryCount === 31) {
    ...
    checks.push({ name: 'routing-table', status: 'fail', detail: `Expected 31 routing entries, found ${entryCount}` });
  }

  // 6. GSD directory structure: commands/, workflows/, references/, scaffolds/
  const gsdDirs = [
    { name: 'commands', path: 'commands', expected: 31 },
    { name: 'workflows', path: 'workflows', expected: 31 },
    { name: 'references', path: 'references' },
    { name: 'scaffolds', path: 'scaffolds' },
  ];
  ...
  const expectedAgents = [
    ...
    'fp-docs-ug-writer', 'fp-docs-ug-validator',
  ];
  const missingAgents = [];
  ...
  // 9. User-guide scaffold: check if user-guide directory exists in docs root
  if (codebaseRoot) {
    const docsInfo = paths.getDocsRoot(codebaseRoot);
    if (docsInfo.path) {
      const ugPath = path.join(docsInfo.path, 'user-guide');
      if (fs.existsSync(ugPath)) {
        checks.push({ name: 'user-guide-scaffold', status: 'pass', detail: `User guide scaffold at ${ugPath}` });
      } else {
        checks.push({ name: 'user-guide-scaffold', status: 'warn', detail: `User guide scaffold not found at ${ugPath} -- will auto-bootstrap on first ug-* command` });
      }
    } else {
      checks.push({ name: 'user-guide-scaffold', status: 'skip', detail: 'Skipped -- docs root not available' });
    }
  } else {
    checks.push({ name: 'user-guide-scaffold', status: 'skip', detail: 'Skipped -- codebase root not available' });
  }
