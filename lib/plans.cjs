 ...
 *
 * Stores execution plans as JSON in {docs-root}/.fp-docs/plans/
 * and research analysis documents in {docs-root}/.fp-docs/analyses/.
...
/**
 * Get the plans directory path for a given docs root.
 *
 * @param {string} docsRoot - The docs root directory
 * @returns {string} Absolute path to .fp-docs/plans/
 */
function getPlansDir(docsRoot) {
  return path.join(docsRoot, '.fp-docs', PLANS_DIR);
}

/**
 * Get the analyses directory path for a given docs root.
 *
 * @param {string} docsRoot - The docs root directory
 * @returns {string} Absolute path to .fp-docs/analyses/
 */
function getAnalysesDir(docsRoot) {
  return path.join(docsRoot, '.fp-docs', ANALYSES_DIR);
}
 ...
 */
function savePlan(plan, docsRootPath) {
  ...
  const plansDir = getPlansDir(docsRoot);
  ensureDir(plansDir);
 ...
 * Otherwise, resolves to {docsRoot}/.fp-docs/plans/{idOrPath}.json.
 *
 ...
 */
function loadPlan(idOrPath, docsRootPath) {
  let planPath;
  if (path.isAbsolute(idOrPath)) {
    ...
    const docsRoot = docsRootPath || resolveDocsRoot();
    if (!docsRoot) return null;
    planPath = path.join(getPlansDir(docsRoot), `${idOrPath}.json`);
  }
 ...
 */
function listPlans(docsRootPath) {
  const docsRoot = docsRootPath || resolveDocsRoot();
  if (!docsRoot) return [];
  const plansDir = getPlansDir(docsRoot);
 ...
 */
function updatePlan(idOrPath, updates, docsRootPath) {
  const plan = loadPlan(idOrPath, docsRootPath);
  if (!plan) return null;
 ...
 */
function prunePlans(docsRootPath) {
  const docsRoot = docsRootPath || resolveDocsRoot();
  if (!docsRoot) return { pruned: 0, remaining: 0 };
  const plansDir = getPlansDir(docsRoot);
 ...
 */
function saveAnalysis(content, operation, docsRootPath) {
  const docsRoot = docsRootPath || resolveDocsRoot();
  if (!docsRoot) { return null; }

  const analysesDir = getAnalysesDir(docsRoot);
 ...
 * @returns {{ pruned: number, remaining: number }}
 */
function pruneAnalyses(docsRootPath) {
  const docsRoot = docsRootPath || resolveDocsRoot();
  if (!docsRoot) return { pruned: 0, remaining: 0 };
  const analysesDir = getAnalysesDir(docsRoot);
