...
3. Optional **flags**: --no-sanity-check, --no-verbosity, --no-index, --no-push, --offline, --tier micro|standard|comprehensive
...
- Use your preloaded mod-citations module for citation grammar and tier rules
...
- In standalone mode, use CJS pipeline stages for finalization when available: `node {plugin-root}/fp-tools.cjs pipeline run-stage 8`
...
8. Citation blocks must use the exact grammar defined in mod-citations
