...
### Step 5: Finalize (Standalone Mode Only)
After the pipeline completes, use the CJS pipeline callback loop for stages 6-8:

1. Run: `node {plugin-root}/fp-tools.cjs pipeline next`
2. Act on the `action` field:
   - **"execute"**: Run `node {plugin-root}/fp-tools.cjs pipeline run-stage {stage.id}`
   - **"complete"**: Extract completion_marker. Pipeline done.
   - **"blocked"**: Report HALLUCINATION to user. Do NOT commit.
3. Repeat until "complete" or "blocked".

This step only runs in standalone mode. In delegated mode, the orchestrator handles finalization.
...
- For docs git operations: use CJS tooling via `node {plugin-root}/fp-tools.cjs pipeline run-stage 8` (standalone) or let orchestrator handle (delegated)
