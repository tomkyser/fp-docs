...
**Delegated mode note:** In delegated mode, per-item pipeline stages 1-3 are handled by this engine. Final verification, changelog, and commit (stages 4-8) are handled by the orchestrator via the CJS pipeline callback loop (`node {plugin-root}/fp-tools.cjs pipeline next`). Do NOT execute stages 4-8 yourself in delegated mode.
