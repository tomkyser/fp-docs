...
all affected docs. Delegates to specialized agents for each phase: scope assessment, research,
planning, primary modification (batch-aware with parallel modifiers), verbosity enforcement,
...
```
Parse JSON for: operation, agent, target_files, pipeline_config, feature_flags, changed_files.
...
```bash
SCOPE=$(node "${CLAUDE_PLUGIN_ROOT}/fp-tools.cjs" scope-assess auto-update "$ARGUMENTS")
...
The scope assessment uses the changed_files list to determine:
- How many doc files are affected (drives parallelism in write phase)
- Whether batch waves are needed (9+ targets)
- How many researchers to spawn

If trackerRequired:
```bash
TRACKER_ID=$(node "${CLAUDE_PLUGIN_ROOT}/fp-tools.cjs" tracker create --command auto-update --complexity ${complexity})
```
</step>
...
Agent(
  prompt="Design execution strategy for auto-update operation.
    ...
    Targets: {targets from scope-assess}
    ...
    Classify scope: single-file, multi-file, or batch based on target count.
    Assign batch waves if 9+ targets (max 5 concurrent per wave).
...
## 5. Write Phase (Primary Operation Only -- Batch-Aware)
```bash
MODIFIER_MODEL=$(node "${CLAUDE_PLUGIN_ROOT}/fp-tools.cjs" resolve-model fp-docs-modifier --raw)
```

Scope determines parallelism:
- 1 target: single agent spawn
- 2-8 targets: parallel agent spawns (one per target)
- 9+ targets: batched waves (max 5 concurrent per wave)

For each batch assignment, spawn modifier agent:
```
Agent(
  prompt="Execute auto-update operation -- PRIMARY OPERATION ONLY.
    ...
    6. Update links for any docs created or removed.
    7. If --visual flag: visual verification for each updated doc page.
...
```
Agent(
  prompt="Enforce verbosity on files modified by auto-update operation.
    Target files: {all files from write phase -- merged across batches}
    Tracker: {TRACKER_ID or 'none'}
...
```
Agent(
  prompt="Enforce citations on files modified by auto-update operation.
    Target files: {all files from write phase -- merged across batches}
    Tracker: {TRACKER_ID or 'none'}
...
```
Agent(
  prompt="Enforce API references on files modified by auto-update operation.
...
#   Stage 6: Changelog update (single entry covers all updated files)
#   Stage 7: Index update (may need index agent spawn for structural changes)
...
<success_criteria>
- [ ] All documentation-relevant code changes identified via git diff + source-map
- [ ] Affected docs updated to reflect source changes
- [ ] New docs created for new source files
- [ ] Primary operation completed by modifier agent(s) (step 5, batch-aware)
