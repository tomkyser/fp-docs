...
REMOVED notices and removes from indexes. Updates cross-references and appendices. Delegates
to specialized agents for each phase: scope assessment, research, planning, primary modification,
...
```bash
SCOPE=$(node "${CLAUDE_PLUGIN_ROOT}/fp-tools.cjs" scope-assess deprecate "$ARGUMENTS")
...
If trackerRequired:
```bash
TRACKER_ID=$(node "${CLAUDE_PLUGIN_ROOT}/fp-tools.cjs" tracker create --command deprecate --complexity ${complexity})
```
</step>
...
```
Agent(
  prompt="Execute deprecate operation -- PRIMARY OPERATION ONLY.
    Target: {target}
    Plan: {plan-file-path}
    Flags: {flags}
    Tracker: {TRACKER_ID or 'none'}
...
```
Agent(
  prompt="Enforce verbosity on files modified by deprecate operation.
...
```
Agent(
  prompt="Enforce citations on files modified by deprecate operation.
    Target files: {files from write phase}
    Tracker: {TRACKER_ID or 'none'}
...
```
Agent(
  prompt="Enforce API references on files modified by deprecate operation.
    Target files: {files from write phase}
    Tracker: {TRACKER_ID or 'none'}
